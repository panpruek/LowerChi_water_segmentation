...
BASE_DIR = r"D:\DL_FN2569\DATA\Model\S2Blackout_comparison\ResultDRY\OPENS2" 
OUTPUT_DIR = r"D:\DL_FN2569\DATA\Model\S2Blackout_comparison\ResultDRY\OPENS2\y3220_x4600Individual_Patches"

# 🎯 ระบุ Path ของภาพต้นแบบ (โค้ดจะดึงพิกัด y และ x จากชื่อไฟล์นี้โดยอัตโนมัติ)
REFERENCE_IMAGE_PATH = r"D:\DL_FN2569\DATA\Data\Sen-2\02Processed\Dry\zone_C\water patch folder\image\Sen2_Dry_NDVI_y3220_x4600.png"
...
def parse_model_info(folder_name):
    """สกัดชื่อ Model, Loss, Event, S2 และ Rate จากชื่อโฟลเดอร์ เพื่อประกอบร่างชื่อไฟล์"""
    ...
    # ปรับให้จับคำว่า OPEN/CLOSE เฉยๆ ด้วย (เพราะโฟลเดอร์คุณชื่อ OPEN_Dry)
    if 'CLOSES2' in folder_upper or '_CLOSE_' in folder_upper: s2 = 'CloseS2'
    elif 'OPENS2' in folder_upper or '_OPEN_' in folder_upper: s2 = 'OpenS2'
    else: s2 = 'UnknownS2'
    
    # 🎯 เพิ่มการดักจับคำว่า Rate (เช่น Rate30, Rate40)
    rate_match = re.search(r'(Rate\d+)', folder_name, re.IGNORECASE)
    rate = rate_match.group(1).capitalize() if rate_match else 'UnknownRate'
    
    parts = folder_name.split('_')
    event = parts[-1] if len(parts) > 0 else 'UnknownEvent'
    
    # นำตัวแปร rate เข้ามาประกอบในชื่อไฟล์ด้วย
    return f"{model}_{loss}_{rate}_{s2}_{event}"
